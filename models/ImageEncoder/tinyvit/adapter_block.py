...
class TinyViTAdapterBlock(nn.Module):
    ...
    def __init__(self, args, dim, input_resolution, num_heads, window_size=7,
        ...
        # adapter_dim 用于控制 Adapter 的隐藏层维度，但输入维度应该是 dim
        ...
        # Adapter 的输入维度应该是 dim（实际特征维度），而不是 adapter_dim
        # adapter_dim 用于控制隐藏层维度（通过 mlp_ratio 参数）
        # 但这里 Adapter 的 mlp_ratio 是固定的 0.25，所以我们需要调整
        # 如果 mid_dim 不为 None，我们用它来设置 mlp_ratio，使得隐藏层维度 = mid_dim
        if args.mid_dim is not None:
            # 计算需要的 mlp_ratio 使得隐藏层维度 = mid_dim
            adapter_mlp_ratio = args.mid_dim / dim
        else:
            adapter_mlp_ratio = 0.25  # 默认值
        
        self.MLP_Adapter = Adapter(dim, mlp_ratio=adapter_mlp_ratio, skip_connect=False)  # MLP-adapter, no skip connection
        self.Space_Adapter = Adapter(dim, mlp_ratio=adapter_mlp_ratio)  # with skip connection
        self.Depth_Adapter = Adapter(dim, mlp_ratio=adapter_mlp_ratio, skip_connect=False)  # no skip connection
